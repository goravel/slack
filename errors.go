...
var (
	...
	MarshalPayload     = errors.New("slack channel: failed to marshal payload for %T: %v").SetModule(Module)
	UnmarshalPayload   = errors.New("slack channel: failed to unmarshal payload: %v").SetModule(Module)
	PostMessageFailed  = errors.New("slack channel: chat.postMessage failed: %v").SetModule(Module)
